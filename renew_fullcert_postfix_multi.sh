...
#   証明書         ：multidomain.crt
#   秘密鍵         ：multidomain.key
#   CA証明書       ：multidomain.ca-bundle
#   証明書+CA証明書：multidomain.crt-ca-bundle
#
...
if [ "$CNT" -le 30 ]; then
  # 設定を取り出す
  DOMAIN=`openssl x509 -noout -text -in $FULLCERT | grep "DNS"`
  DOMAIN=`echo $DOMAIN | sed -e 's/DNS://g' -e 's/ //g'`
  ...
  # ドキュメントルート
  WEBROOT=`cat /etc/httpd/conf.d/virtualhost.conf | grep DocumentRoot | awk '{print $2}' | uniq`

  # 秘密鍵設定
  MULTIDOMAIN=multidomain
  KEY="${CONFDIR}${MULTIDOMAIN}.key"
  if [ ! -f $KEY ]; then
    echo "'$KEY'がありません。"
    exit 1
  fi
...
##      openssl req -new -key $KEY -sha256 -nodes -subj "/CN=$DOMAIN" > $CSR
  ...
  # 証明書設定
  CERT="${CONFDIR}${MULTIDOMAIN}.crt"
  CERTCA="${CONFDIR}${MULTIDOMAIN}.ca-bundle"
